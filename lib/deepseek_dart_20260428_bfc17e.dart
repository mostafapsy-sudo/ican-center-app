...
class _HomePageState extends State<HomePage> {
  ...
  List<TabItem> get tabs {
    ...
    if (widget.role == 'manager' || widget.role == 'senior') {
      list.add(const TabItem('متابعة السينيور', Icons.verified_rounded, PlaceholderPage(title: 'متابعة السينيور', subtitle: 'سيتم اختيار الطفل الحقيقي من Firebase.')));
    }
...
class _AddChildCardState extends State<AddChildCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return SectionCard(
      title: 'إضافة طفل جديد - حفظ حقيقي في Firebase',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          ...
          const SizedBox(height: 8),
          TextField(controller: parentController, decoration: inputDecoration('اسم ولي الأمر')),
          const SizedBox(height: 8),
          ...
          FilledButton.icon(
            ...
            icon: saving ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.add),
            label: Text(saving ? 'جار الحفظ...' : 'إضافة الطفل وحفظه'),
          ),
        ],
      ),
    );
...
// EditChildDialog added for editing children
class EditChildDialog extends StatefulWidget {
  final String childId;
  final Map<String, dynamic> childData;

  const EditChildDialog({super.key, required this.childId, required this.childData});
...
class _EditChildDialogState extends State<EditChildDialog> {
  ...
  late final TextEditingController parentController;
  ...
  void initState() {
    ...
    diagnosisController = TextEditingController(text: widget.childData['diagnosis'] ?? '');
    parentController = TextEditingController(text: widget.childData['parentName'] ?? '');
    ...
    program = widget.childData['program'] ?? 'بورتاج + لوفاس';
    selectedParentId = widget.childData['parentId']?.toString();
    selectedParentName = widget.childData['parentName']?.toString();
    selectedParentEmail = widget.childData['parentEmail']?.toString();
    if (widget.childData['dateOfBirth'] is Timestamp) {
      birthDate = (widget.childData['dateOfBirth'] as Timestamp).toDate();
    }
  }
  ...
  Future<void> save() async {
    ...
    setState(() => saving = true);
    try {
      await FirebaseFirestore.instance.collection('children').doc(widget.childId).update({
        'name': name,
        'dateOfBirth': Timestamp.fromDate(birthDate!),
        'ageText': ageText,
        'program': program,
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        return Row(
          children: [
            Expanded(child: StatMiniCard(title: 'عدد الأطفال الحقيقي', value: '$count')),
            const SizedBox(width: 8),
            const Expanded(child: StatMiniCard(title: 'متوسط التحسن', value: '0%')),
            const SizedBox(width: 8),
            const Expanded(child: StatMiniCard(title: 'العاملون', value: '0')),
            const SizedBox(width: 8),
            const Expanded(child: StatMiniCard(title: 'التقارير', value: '0')),
          ],
        );
      },
    );
...
class ChildrenListCard extends StatelessWidget {
  ...
  Future<void> deleteChild(BuildContext context, String docId, Map<String, dynamic> child) async {
    final ok = await confirmDialog(context, 'نقل للسلة', 'هل تريد نقل هذا الطفل وجميع أهدافه إلى السلة؟');
    if (!ok) return;
    try {
      await moveDocumentToTrash(
        ...
        docId: docId,
        data: child,
        itemTitle: 'طفل: ${child['name'] ?? 'بدون اسم'}',
        itemType: 'child',
      );
      final goalsSnapshot = await FirebaseFirestore.instance
          .collection('goals')
          .where('childId', isEqualTo: docId)
          .get();
      for (final goalDoc in goalsSnapshot.docs) {
        await moveDocumentToTrash(
          ...
          docId: goalDoc.id,
          data: goalDoc.data(),
          itemTitle: 'هدف: ${goalDoc.data()['text'] ?? ''}',
          itemType: 'goal',
        );
        final weeklySnapshot = await FirebaseFirestore.instance
            .collection('weeklyPlans')
            .where('goalId', isEqualTo: goalDoc.id)
            .get();
        for (final weeklyDoc in weeklySnapshot.docs) {
          await moveDocumentToTrash(
            collectionName: 'weeklyPlans',
            docId: weeklyDoc.id,
            data: weeklyDoc.data(),
            itemTitle: 'برنامج أسبوعي لهدف',
            itemType: 'weeklyPlan',
          );
        }
      }
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم نقل الطفل وجميع أهدافه إلى السلة')));
    } catch (e) {
      if (!context.mounted) return;
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () => showDialog(
                          context: context,
                          builder: (_) => EditChildDialog(childId: doc.id, childData: child),
                        ),
                      ),
                      IconButton(
                        icon: const Icon(Icons.delete_rounded, color: Colors.red),
                        onPressed: () => deleteChild(context, doc.id, child),
                      ),
                    ],
                  ),
                ),
...
class _BackupCardState extends State<BackupCard> {
  ...
  Future<void> exportBackupToFile() async {
    ...
    try {
      ...
      final fileName = 'ican_backup_${DateTime.now().toIso8601String().replaceAll(':', '-')}.json';
      ...
      if (result != null && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم حفظ النسخة في: $result')));
      } else if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم إلغاء الحفظ')));
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('فشل تصدير النسخة: $e')));
    } finally {
      if (mounted) setState(() => loading = false);
    }
  }
  ...
  Future<void> shareBackup() async {
    ...
    try {
      ...
      await Share.shareXFiles([XFile(file.path)], text: 'نسخة احتياطية لنظام ICAN');
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> importBackupFromFile() async {
    ...
    try {
      ...
      if (fileBytes == null) throw Exception('الملف فارغ');
      final jsonString = utf8.decode(fileBytes);
      await restoreBackupJson(jsonString);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم استرداد النسخة الاحتياطية بنجاح')));
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return SectionCard(
      ...
      child: Wrap(
        ...
        children: [
          FilledButton.icon(
            onPressed: loading ? null : exportBackupToFile,
            icon: const Icon(Icons.backup),
            label: Text(loading ? 'جار...' : 'تصدير JSON'),
          ),
          OutlinedButton.icon(
            onPressed: loading ? null : shareBackup,
            icon: const Icon(Icons.share),
            label: const Text('مشاركة'),
          ),
          OutlinedButton.icon(
            onPressed: loading ? null : importBackupFromFile,
...
// AssessmentPage and related classes remain largely unchanged but goal deletion is updated
...
class _AddGoalCardState extends State<AddGoalCard> {
  ...
  Future<void> saveGoal() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حفظ الهدف وربطه بالطفل في Firebase')));
    } on FirebaseException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('خطأ Firebase: ${e.code} - ${e.message ?? ''}')));
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return SectionCard(
      title: 'إضافة هدف للطفل: ${widget.childName}',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          TextField(
...
class _GoalsListCardState extends State<GoalsListCard> {
  ...
  Future<void> deleteGoal(String goalId, Map<String, dynamic> goal) async {
  ...
  Widget build(BuildContext context) {
    ...
    return PageWrap(
      children: [
        ...
        const SizedBox(height: 12),
        SectionCard(
          title: isParent ? 'إضافة ولي أمر' : 'إضافة عامل / مستخدم جديد',
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
...
class UsersListCard extends StatelessWidget {
  ...
  Future<void> deleteUser(BuildContext context, String docId, Map<String, dynamic> user) async {
  ...
  @override
  Widget build(BuildContext context) {
    if (!isCurrentUserManager()) return const SizedBox.shrink();
                ...
                child: ListTile(
                  ...
                  trailing: PopupMenuButton<String>(
                    onSelected: (value) async {
                      ...
                      if (value == 'delete') {
                        await deleteUser(context, doc.id, user);
                      }
                    },
                    itemBuilder: (_) => const [
                      PopupMenuItem(value: 'edit', child: Text('تعديل')),
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, userSnapshot) {
        ...
        final childId = userData['linkedChildId'];
        final childName = userData['linkedChildName'] ?? 'الطفل';
        ...
        return PageWrap(
          children: [
            HeroBox(
              ...
              subtitle: 'متابعة خاصة للطفل: $childName — عرض فقط بدون تعديل أو حذف.',
            ),
            const SizedBox(height: 12),
            ParentChildInfoCard(childId: childId.toString()),
            ParentGoalsCard(childId: childId.toString(), childName: childName.toString()),
            ParentWeeklyCard(
...
class AttendanceRulesCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SectionCard(
      title: 'قواعد الحضور والإجازات المعتمدة',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
...
class _ManagerAttendanceViewState extends State<ManagerAttendanceView> {
  String selectedMonth = formatMonthKey(DateTime.now());
  String selectedEmail = 'كل العاملين';
  final monthController = TextEditingController(text: formatMonthKey(DateTime.now()));
  ...
  void applyMonthFilter() {
    setState(() => selectedMonth = monthController.text.trim());
  }
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: TextField(
                  decoration: inputDecoration('فلتر الشهر بصيغة YYYY-MM'),
                  controller: monthController,
                  onSubmitted: (_) => applyMonthFilter(),
                ),
              ),
              const SizedBox(width: 8),
              FilledButton(onPressed: applyMonthFilter, child: const Text('تطبيق')),
            ],
          ),
          const SizedBox(height: 8),
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            children: [
              ...
              OutlinedButton.icon(
                onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('استخدم زر تسجيل/تعديل يوم لعامل لتسجيل الغياب أو تعديل الحضور والانصراف.')),
                ),
                icon: const Icon(Icons.info_outline_rounded),
                label: const Text('طريقة التعديل'),
              ),
            ],
          ),
          const SizedBox(height: 12),
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              final quarterKey = selectedMonth.length >= 7 ? quarterKeyForDateKey('$selectedMonth-01') : quarterKeyForDateKey(formatDateKey(DateTime.now()));
              final quarterSummary = buildQuarterSummary(records, quarterKey);

              return Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  AttendanceSummaryPanel(summary: summary, quarterSummary: quarterSummary, quarterKey: quarterKey),
                  const SizedBox(height: 10),
                  ...docs.map((doc) => AttendanceListTile(
                        item: doc.data(),
                        onEdit: () => showDialog(
                          context: context,
                          builder: (_) => AttendanceEditDialog(existingDocId: doc.id, existingData: doc.data()),
                        ),
                      )),
                ],
              );
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text('ملخص الخصومات', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          ...
          InfoLine(label: 'الإضافي', value: '${summary.totalOvertimeMinutes} دقيقة × 3 = ${summary.overtimePaidMinutes} دقيقة مدفوعة = ${summary.overtimeAmount.toStringAsFixed(2)} جنيه'),
          InfoLine(label: 'خصم الغياب', value: '${summary.absenceDeductionDays} يوم = ${summary.absenceDeductionAmount.toStringAsFixed(2)} جنيه'),
          if (summary.baseSalary > 0) InfoLine(label: 'الراتب الأساسي', value: '${summary.baseSalary.toStringAsFixed(2)} جنيه'),
          if (summary.baseSalary > 0) InfoLine(label: 'أجر اليوم الفعلي', value: '${summary.dailyRate.toStringAsFixed(2)} جنيه'),
          if (summary.baseSalary > 0) InfoLine(label: 'صافي تقديري', value: '${summary.netSalary.toStringAsFixed(2)} جنيه'),
...
class AttendanceListTile extends StatelessWidget {
  ...
  const AttendanceListTile({super.key, required this.item, this.onEdit});
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        isThreeLine: true,
        trailing: onEdit == null ? null : IconButton(icon: const Icon(Icons.edit_rounded), onPressed: onEdit),
      ),
    );
  }
}
...
Future<void> moveDocumentToTrash({
  ...
  final trashRef = FirebaseFirestore.instance.collection('deletedItems').doc();
...
Future<void> restoreTrashDocument(String trashId, Map<String, dynamic> trashData) async {
  final collectionName = (trashData['collectionName'] ?? '').toString();
  final originalDocId = (trashData['originalDocId'] ?? '').toString();
  ...
  batch.delete(FirebaseFirestore.instance.collection('deletedItems').doc(trashId));
  await batch.commit().timeout(const Duration(seconds: 12));
}

Future<String> createBackupJson() async {
  final collections = ['children', 'users', 'goals', 'weeklyPlans', 'attendance', 'deletedItems'];
...
class _TrashPageState extends State<TrashPage> {
  ...
  Future<void> restoreAll(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) async {
    ...
    try {
      ...
      selectedIds.clear();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم استعادة جميع العناصر (${docs.length})')));
    } finally {
      if (mounted) setState(() => loading = false);
    }
  }
  ...
  Future<void> deleteSelectedForever(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) async {
    final ok = await confirmDialog(context, 'حذف نهائي', 'سيتم حذف العناصر المحددة نهائيًا من السلة. هل أنت متأكد؟');
    if (!ok) return;
    setState(() => loading = true);
    try {
      final batch = FirebaseFirestore.instance.batch();
  ...
  Future<void> deleteAllForever(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) async {
    final ok = await confirmDialog(context, 'حذف نهائي للكل', 'سيتم حذف جميع العناصر في السلة نهائيًا. هل أنت متأكد؟');
    if (!ok) return;
    setState(() => loading = true);
    try {
      final batch = FirebaseFirestore.instance.batch();
      ...
      selectedIds.clear();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حذف جميع العناصر نهائيًا')));
    } finally {
      if (mounted) setState(() => loading = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    return PageWrap(
      children: [
        const HeroBox(title: 'السلة المتقدمة', subtitle: 'استعادة عنصر أو مجموعة عناصر أو حذفها نهائيًا.'),
        const SizedBox(height: 12),
        SectionCard(
          title: 'العناصر المحذوفة',
          child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            stream: FirebaseFirestore.instance.collection('deletedItems').orderBy('deletedAt', descending: true).snapshots(),
            builder: (context, snapshot) {
              if (snapshot.hasError) return Text('خطأ في قراءة السلة: ${snapshot.error}');
              if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
              final docs = snapshot.data?.docs ?? [];
              if (docs.isEmpty) return const Text('السلة فارغة.');

              final allSelected = selectedIds.length == docs.length;
              return Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  Wrap(
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        onPressed: loading ? null : () => restoreAll(docs),
                      ...
                      OutlinedButton.icon(
                        onPressed: loading ? null : () => deleteAllForever(docs),
                        icon: const Icon(Icons.delete_sweep),
                        label: const Text('حذف نهائي الكل'),
                      ),
                    ],
                  ),
                  const SizedBox(height: 12),
                  ...docs.map((doc) {
                    ...
                    if (deletedAt is Timestamp) {
                      deletedDate = deletedAt.toDate().toString().split(' ').first;
                    }
                    return Card(
                      margin: const EdgeInsets.only(bottom: 8),
