      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: SizedBox(
            width: 700,
            child: TextField(
              controller: TextEditingController(text: backup),
              maxLines: 14,
              readOnly: true,
              decoration: inputDecoration('انسخ هذا النص واحفظه في ملف آمن'),
            ),
          ),
          actions: [
      ...
      child: Wrap(
        ...
        children: [
          FilledButton.icon(
            ...
            label: Text(loading ? 'جار إنشاء النسخة...' : 'إنشاء نسخة احتياطية'),
          ),
          OutlinedButton.icon(
            onPressed: loading ? null : () => showDialog(context: context, builder: (_) => const BackupRestoreDialog()),
            icon: const Icon(Icons.restore),
            label: const Text('استرداد نسخة احتياطية'),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, userSnapshot) {
        ...
        // الربط الجديد: البحث في children بحقل parentId = parentDocId
        return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          stream: FirebaseFirestore.instance
              .collection('children')
          ...
          builder: (context, childrenSnap) {
            ...
            final childDocs = childrenSnap.data?.docs ?? [];
            ...
            return PageWrap(
              children: [
                HeroBox(
                  title: 'مرحبًا بولي الأمر',
                  subtitle: 'متابعة خاصة — عدد الأطفال المرتبطين: ${childDocs.length}',
                ),
                ...childDocs.expand((childDoc) {
