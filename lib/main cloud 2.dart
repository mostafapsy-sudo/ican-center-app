          ...
          builder: (context, usersSnap) {
            ...
            return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, plansSnap) {
                ...
                final reportsCount = planDocs.length;
                // متوسط achievementPercent
                ...
                return Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Expanded(child: StatMiniCard(title: 'أهداف الأسبوع', value: '$reportsCount')),
                  ],
                );
              },
            );
    ...
    return PageWrap(
      children: [
        const HeroBox(
          ...
          subtitle: 'اختَر الطفل ثم راجع الأهداف واعتمدها أو أرجع تغذية راجعة.',
        ),
        const SizedBox(height: 12),
        SectionCard(
          title: 'اختيار الطفل والأسبوع',
          child: Column(
            children: [
              StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                stream: FirebaseFirestore.instance.collection('children').orderBy('createdAt', descending: true).snapshots(),
...
class SeniorWeeklyPlanCard extends StatelessWidget {
  final String childId;
  final String childName;
  final int year;
  final int month;
  ...
  Widget build(BuildContext context) {
    return SectionCard(
      title: 'أهداف $childName - الأسبوع $week / $month / $year',
      child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance
            .collection('weeklyPlans')
            .where('childId', isEqualTo: childId)
        ...
        builder: (context, snapshot) {
          ...
          final docs = snapshot.data?.docs ?? [];
          if (docs.isEmpty) return const Text('لا توجد أهداف لهذا الأسبوع بعد.');

          // تجميع حسب نوع الجلسة
          final grouped = <String, List<QueryDocumentSnapshot<Map<String, dynamic>>>>{};
          for (final doc in docs) {
            final session = (doc.data()['sessionType'] ?? 'غير محدد').toString();
            grouped.putIfAbsent(session, () => []);
            grouped[session]!.add(doc);
          }

          final total = docs.length;
          final approved = docs.where((d) => d.data()['seniorApproved'] == true).length;
            ...
            children: [
                ...
                child: Row(
                  ...
                  children: [
                    Column(children: [
                      ...
                      const Text('إجمالي الأهداف', style: TextStyle(fontSize: 12, color: Colors.black54)),
                    ]),
                    Column(children: [
                      Text('$approved', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.green)),
                      const Text('معتمدة', style: TextStyle(fontSize: 12, color: Colors.black54)),
                    ]),
                    Column(children: [
                      Text('${total - approved}', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.orange.shade700)),
                      const Text('تنتظر الاعتماد', style: TextStyle(fontSize: 12, color: Colors.black54)),
                    ]),
                  ],
                ),
              ),
              ...
              if (approved < total)
                FilledButton.icon(
                  ...
                  onPressed: () async {
                    final ok = await confirmDialog(context, 'اعتماد كل الجلسة', 'هل تريد اعتماد جميع الأهداف في هذا الأسبوع؟');
                    if (!ok) return;
                    final batch = FirebaseFirestore.instance.batch();
                    for (final doc in docs) {
                      batch.update(doc.reference, {
                        'seniorApproved': true,
                        'seniorName': currentUserDisplayName(),
                        'seniorApprovedAt': FieldValue.serverTimestamp(),
                      });
                    }
                    await batch.commit();
                    if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم اعتماد جميع الأهداف')));
                  },
                  icon: const Icon(Icons.verified_rounded),
                  label: const Text('اعتماد جميع الأهداف'),
                ),
              const SizedBox(height: 12),
              // القائمة مجمّعة حسب الجلسة
              ...grouped.entries.map((entry) {
                final sessionType = entry.key;
                final sessionDocs = entry.value;
                return Container(
                  margin: const EdgeInsets.only(bottom: 14),
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: [
                      Text('نوع الجلسة: $sessionType', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 8),
                      ...sessionDocs.map((doc) {
                        final item = doc.data();
                        final isApproved = item['seniorApproved'] == true;
                        final achievement = item['achievementPercent'] ?? 0;
                        return Card(
                          color: isApproved ? const Color(0xFFE7FBEA) : Colors.white,
                          margin: const EdgeInsets.only(bottom: 8),
                          child: Padding(
                            padding: const EdgeInsets.all(10),
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 6),
                                Text(
                                  'الأخصائي: ${item['goalAuthor'] ?? '-'} | المساعدة: ${item['promptLevel'] ?? '-'} | التعزيز: ${item['reinforcementSchedule'] ?? '-'} | الإنجاز: $achievement%',
                                  style: const TextStyle(color: Colors.black54, fontSize: 12),
                                ),
                                if (isApproved)
                                  Text(
                                    'اعتمدها: ${item['seniorName'] ?? '-'}',
                                    ...
                                    if (!isApproved)
                                      FilledButton.icon(
                                        ...
                                        onPressed: () async {
                                          ...
                                          if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم اعتماد الهدف')));
                                        },
                                        icon: const Icon(Icons.verified_rounded, size: 16),
                                        label: const Text('اعتماد', style: TextStyle(fontSize: 13)),
                                      )
                                    else
                                      OutlinedButton.icon(
                                        ...
                                        onPressed: () async {
                                          ...
                                          if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم إلغاء اعتماد الهدف')));
                                        },
                                        icon: const Icon(Icons.undo_rounded, size: 16),
                                        label: const Text('إلغاء الاعتماد', style: TextStyle(fontSize: 13)),
                                      ),
                                    // حقل تحديث نسبة الإنجاز
                                    OutlinedButton.icon(
                                      style: OutlinedButton.styleFrom(
                                        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                                      ),
                                      onPressed: () => _showUpdateAchievementDialog(context, doc),
                                      icon: const Icon(Icons.edit_rounded, size: 16),
                                      label: const Text('تحديث الإنجاز', style: TextStyle(fontSize: 13)),
                                    ),
                                  ],
                                ),
                              ],
  ...
  void _showUpdateAchievementDialog(BuildContext context, QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    final item = doc.data();
    final controller = TextEditingController(text: (item['achievementPercent'] ?? 0).toString());
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text('تحديث الإنجاز: ${(item['goalText'] ?? '').toString().length > 40 ? '${(item['goalText'] ?? '').toString().substring(0, 40)}...' : item['goalText'] ?? ''}'),
        content: TextField(
          controller: controller,
          keyboardType: TextInputType.number,
          decoration: inputDecoration('نسبة الإنجاز % (0-100)'),
        ),
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              await doc.reference.update({
                'achievementPercent': clamped,
                'lastAchievementPercent': clamped,
                'seniorUpdatedAt': FieldValue.serverTimestamp(),
                'seniorUpdatedBy': currentUserDisplayName(),
              });
              // تحديث الهدف الأصلي أيضًا
              final goalId = item['goalId'];
              if (goalId != null) {
                await FirebaseFirestore.instance.collection('goals').doc(goalId.toString()).update({
                  'lastAchievementPercent': clamped,
                  'lastWeeklyUpdateAt': FieldValue.serverTimestamp(),
                });
              }
              if (ctx.mounted) Navigator.pop(ctx);
              if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم تحديث الإنجاز إلى $clamped%')));
            },
